...
<body class="font-sans antialiased">
    <div class="min-h-screen bg-gray-100">
        ...
        @if (Auth::user()->employee->position === 'registrar')
        @include('registrar.sidenav')
        @endif

        @if (Auth::user()->employee->position === 'teacher')c
        @include('professor.sidenav')
        @endif

        @if (Auth::user()->employee->position === 'program_head')
        @include('program_head.sidenav')
        @endif

        @if (Auth::user()->employee->position === 'treasury')
