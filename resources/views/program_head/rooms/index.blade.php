...
<div class="relative mb-8">
    <select class="peer p-4 pe-9 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none
    focus:pt-6
    focus:pb-2
    ...
    <label class="absolute top-0 start-0 p-4 h-full truncate pointer-events-none transition ease-in-out duration-100 border border-transparent peer-disabled:opacity-50 peer-disabled:pointer-events-none
      ...
      peer-[:not(:placeholder-shown)]:text-gray-500 dark:peer-[:not(:placeholder-shown)]:text-neutral-500">Department Building</label>
  </div>
  <!-- End Floating Select -->
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($rooms as $room)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">{{ $room->building }}</td>
        ...
        <form action="{{ route('rooms.store') }}" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="building" id="building" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                    @error('building')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <div class="relative w-full max-w-md p-6 mx-4 bg-white rounded-lg shadow-xl sm:mx-auto">
        ...
        <form id="editRoomForm" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label for="edit_building" class="block text-sm font-medium text-gray-700">Building</label>
                    <input type="text" name="building" id="edit_building" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                </div>
                <div>
...
<div id="deleteModal" class="fixed inset-0 z-[70] flex items-center justify-center hidden overflow-auto bg-black bg-opacity-50">
    <div class="relative w-full max-w-sm p-6 mx-4 bg-white rounded-lg shadow-xl sm:mx-auto">
        <button onclick="closeDeleteModal()" class="absolute text-gray-400 top-4 right-4 hover:text-gray-600">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>
        <h2 class="mb-4 text-2xl font-bold text-gray-900">Confirm Deletion</h2>
        <p class="mb-6 text-gray-600">Are you sure you want to delete this room? This action cannot be undone.</p>
