            ...
            <div class="p-6">
                <form action="{{ route('rooms.update', $room) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="building" id="building" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('building') border-red-500 @enderror" value="{{ old('building', $room->building) }}" required>
                        @error('building')
                            <p class="mt-2 text-xs italic text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-4">
                        ...
                        <input type="number" name="capacity" id="capacity" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('capacity') border-red-500 @enderror" value="{{ old('capacity', $room->capacity) }}" required min="1">
                        @error('capacity')
                            <p class="mt-2 text-xs italic text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="flex items-center justify-between">
                        <button type="submit" class="px-4 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-600 focus:outline-none focus:shadow-outline">
                            Update Room
                        </button>
